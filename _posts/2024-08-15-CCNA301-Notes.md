...
- The router's interface receives a data packet.
- The interface checks the integrity of the frame (error checking).

#### Decapsulating the Packet:
- The frame header and trailer are stripped off.
- The router extracts the IP packet from the frame.

#### Examining the Packet:
- The router reads the packet's destination IP address.

#### Routing Table Lookup:
- The router searches its routing table using the destination IP address.
- The routing table typically includes network destinations, subnet masks, gateways (next hops), and interface information.

#### Making a Forwarding Decision:
- If the destination IP matches an entry, the router determines the outgoing interface and next hop.
- If no match is found, the router uses a default route if one is configured.
- If no route is found, the packet is dropped, and an ICMP "destination unreachable" message is sent to the source.

#### Encapsulating the Packet:
- The router encapsulates the IP packet into a new frame.
- The new frame header includes the MAC address of the next hop or the final destination on the local network.

#### Forwarding the Packet:
- The router sends the frame out through the chosen interface.
- The packet moves towards its destination, possibly through multiple routers.

### How Routers Connect Multiple Networks 
Routers connect multiple networks by serving as intermediaries that route data packets between different network segments. Here's a detailed explanation of how routers accomplish this:

#### Interfaces
- Multiple Interfaces Routers have multiple network interfaces, each connected to a different network segment. Each interface has its own IP address and subnet, allowing the router to communicate with devices on different networks.

#### Routing Table
- Routing Table Routers maintain a routing table that contains information about various network destinations, including the associated subnet masks, gateways (next hops), and interface information. The routing table is used to determine the best path for forwarding packets to their destination.
