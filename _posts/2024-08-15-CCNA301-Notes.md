...
#### Packet Forwarding Process
   ...
   - The router receives a data packet on one of its interfaces.

   - It checks the integrity of the packet to ensure it hasn't been corrupted.

   - The router decapsulats the frame header and trailer, which reveals the network layer packet (typically an IP packet).

   - The router examines the destination IP address in the packet's header.

   - The router searches its routing table for the destination IP address.

   - It identifies the best matching route based on the longest prefix match (the most specific network).

   - If a matching route is found, the router determines the outgoing interface and the next hop (another router or the destination device itself).

   - If no match is found, the router uses a default route if one is configured. Otherwise, it discards the packet and may send an ICMP "destination unreachable" message to the source.

   - The router encapsulates the IP packet into a new frame for the next network segment.

   - The new frame header includes the MAC address of the next hop or the final destination on the local network.

   - The router sends the frame out through the appropriate interface towards the next hop.

   - This process repeats at each subsequent router until the packet reaches its final destination.

#### Router Modes

| Mode                          | Command                   | Description                                                             |
|-------------------------------|---------------------------|-------------------------------------------------------------------------|
| User EXEC mode                |                           | Limited viewing of configuration. You cannot make changes in this mode  |
| User EXEC mode                | enable                    | Moves to privileged EXEC mode                                           |
| Privileged EXEC mode          |                           | You can see the configuration and move to make changes                  |
| Privileged EXEC mode          | configure terminal        | Moves to global configuration mode. This prompt indicates that you can start making changes |
| Global configuration mode     | interface [type] [number] | Moves to interface configuration mode                                   |
| Interface configuration mode  |                           | Configuration commands for a specific interface                         |
| Global configuration mode     | line [type] [number]      | Moves to line configuration mode                                        |
| Line configuration mode       |                           | Configuration commands for a specific line                              |
| Global configuration mode     | router [protocol]         | Moves to router configuration mode                                      |
| Router configuration mode     |                           | Configuration commands for a routing protocol                           |
| Global configuration mode     | exit                      | Exits to privileged EXEC mode                                           |
| Interface configuration mode  | exit                      | Exits to global configuration mode                                      |
| Line configuration mode       | exit                      | Exits to global configuration mode                                      |
| Router configuration mode     | exit                      | Exits to global configuration mode                                      |
| Privileged EXEC mode          | show running-config       | Displays the current configuration                                      |
| Privileged EXEC mode          | show startup-config       | Displays the configuration stored in NVRAM                              |
| Privileged EXEC mode          | copy running-config startup-config | Saves the current configuration to NVRAM                      |
| Privileged EXEC mode          | reload                    | Restarts the router                                                     |
| Privileged EXEC mode          | debug [option]            | Enables debugging for a specified option                                |
| Privileged EXEC mode          | undebug all               | Disables all debugging                                                  |
| Privileged EXEC mode          | show ip interface brief   | Displays a brief summary of interface status                            |
| Privileged EXEC mode          | show version              | Displays the IOS version and system information                         |
| Global configuration mode     | hostname [name]           | Sets the router hostname                                                |
| Global configuration mode     | enable secret [password]  | Sets the encrypted enable password                                      |
| Global configuration mode     | ip route [destination] [mask] [next-hop] | Configures a static route                                |
| Interface configuration mode  | ip address [address] [mask] | Sets the IP address for an interface                                  |
| Interface configuration mode  | no shutdown               | Enables an interface                                                    |
| Interface configuration mode  | shutdown                  | Disables an interface                                                   |
| Line configuration mode       | password [password]       | Sets the password for a line                                            |
| Line configuration mode       | login                     | Enables password checking on a line                                     |
| Router configuration mode     | network [network]         | Specifies a network for a routing protocol                              |
| Router configuration mode     | no network [network]      | Removes a network from a routing protocol                               |
